...
if __name__ == "__main__":
    ...
    settings_range = range(5, 9 + 1)
    settings_len = len(settings_range)

    max_signal = 0
    max_combination = 0
    for combination in it.permutations(settings_range):
        current_output = 0
        computers = [
            Computer(memory, input=[combination[i]]) for i in range(settings_len)
        ]
        stopped = False
        while not stopped:
            for i in range(settings_len):
                computers[i].input.append(current_output)
                computers[i].run()
                current_output = computers[i].output.pop(0)
                if computers[i].stopped:
                    stopped = True
    ...
    print(f"max_signal={max_signal}, max_combination={max_combination}")
